...
const createResponsePacket = (sequence: number, command: Command) =>
  create(sequence, {
    command,
    response: true,
    flags: ["isResponse", "resetsInactivityTimeout"],
  });

const renderPlatformUsingCoordinates = (
  p: {
    x1: number;
    y1: number;
    x2: number;
...
export default class SpheroMiniSimulator implements Drivable {
  ...
  private contents: {
    ...
    walls: OBB[];
    platforms: {
      mesh: THREE.Mesh<THREE.PlaneGeometry, THREE.MeshToonMaterial>;
  ...
  constructor(
    ...
    platforms.forEach((p) => {
      ...
      let platform = renderPlatformUsingCoordinates(scaled, 0.1);
  ...
  private animate = (time: number) => {
    ...
    let deltaAngleRadians =
      deltaSeconds * this.settings.heading.radiansPerSecond;
    ...
    let paint = 5;
    if (Math.abs(this.settings.heading.radiansPerSecond) > 0) {
    ...
    this.contents.camera.position.lerp(
      this.contents.camera.position.clone().add(deltaPosition),
      0.9,
    );
    ...
    let sphere = this.contents.ball.geometry
      .boundingSphere!.clone()
      .applyMatrix4(this.contents.ball.matrixWorld);
    if (this.contents.walls.some((w) => w.intersectsSphere(sphere))) {
      this.contents.renderer.setAnimationLoop(null);
      throw new Error("You touched one of the boundaries!");
    }
    this.contents.platforms
      .filter(
        (p) =>
          sphere.center.x >= p.coordinates.x1 &&
          sphere.center.x <= p.coordinates.x2 &&
          sphere.center.y >= p.coordinates.y1 &&
          sphere.center.y <= p.coordinates.y2,
      )
      .forEach((p) => {
  ...
  public wake = () => {
    ...
    this.contents.mainLed.color.setRGB(
      this.settings.color.red / 255,
      this.settings.color.green / 255,
      this.settings.color.blue / 255,
    );
    this.contents.mainLed.intensity = CONSTANTS.maxIntensity;
    return new Promise<Packet>((resolve) =>
      setTimeout(
        () => resolve(createResponsePacket(this.sequence++, "power:wake")),
        500,
      ),
    );
  };
  public sleep = () => {
    ...
    return Promise.resolve(
      createResponsePacket(this.sequence++, "power:sleep"),
    );
  };
  ...
  public setBackLed = (brightness: number) => {
    ...
    this.contents.backLed.intensity =
      (brightness / 255) * CONSTANTS.maxIntensity;
    return Promise.resolve(
      createResponsePacket(this.sequence++, "io:rearLightBrightness"),
    );
  };
  public roll = (speed: number, heading: number) => {
    ...
    this.settings.heading = {
      radiansPerSecond: degreesToRadians(
        (speed / 255) * CONSTANTS.maxDegreesPerSecond,
      ),
    ...
    return Promise.resolve(
      createResponsePacket(this.sequence++, "driving:driveWithHeading"),
    );
  };
  public rollTime = (speed: number, heading: number, time: number) =>
    new Promise<Packet>((resolve, reject) =>
      this.roll(speed, heading).then(
        () =>
          setTimeout(() => this.roll(0, heading).then(resolve, reject), time),
        reject,
      ),
    );
  public setStabilization = (enabled: boolean) =>
    Promise.resolve(
      createResponsePacket(this.sequence++, "driving:setStabilization"),
    );
  public resetAim = () =>
    Promise.resolve(createResponsePacket(this.sequence++, "driving:resetAim"));
