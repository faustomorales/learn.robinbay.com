<script lang="ts">
  ...
  const verifier: Verifier = (iframe) => {
    ...
    if (!validations.every((v) => v.message && v.message.startsWith("✅"))) {
      fail("");
    }
  };
</script>
...
<Step {title} {verifier} bind:this={step}>
  <p class="mb-4">
    We now wish to make our application interactive. We want it to compute the
    distances to each food pantry and sort the table accordingly. Below, when
    you create variables and functions, please use the <span class="font-mono"
      >var</span
    >
